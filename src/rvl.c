...
static void rvl_write_chunk_header(RVL_t* self, RVLChunkCode_t code, RVLSize_t size);
static void rvl_write_chunk_payload(RVL_t* self, RVLConstByte_t* data, RVLSize_t size);
...
void rvl_write_DATA(RVL_t* self, RVLConstByte_t* data, RVLSize_t size)
{
    ...
    const int compressedSize = LZ4_compress_HC((char*)data, compressedBuf, size, size, LZ4HC_CLEVEL_MIN);

    rvl_write_chunk_header(self, RVL_CHUNK_CODE_DATA, compressedSize);
    rvl_write_chunk_payload(self, (RVLConstByte_t*)compressedBuf, compressedSize);
    rvl_write_chunk_end(self);
}
...
void rvl_write_TEXT(RVL_t* self, const RVLText_t* arrText, int numText)
{
    for (int i = 0; i < numText; i++) {
        ...
        rvl_write_chunk_header(self, RVL_CHUNK_CODE_TEXT, keySize + valueSize + 1);
        ...
        if (valueSize != 0) {
            rvl_write_chunk_payload(self, (RVLConstByte_t*)text->value, valueSize);
        }
...
void rvl_write_chunk_payload(RVL_t* self, RVLConstByte_t* data, RVLSize_t size)
{
    if (data != NULL && size > 0) {
...
void rvl_write_file(RVL_t* self, RVLConstByte_t* data, RVLSize_t size)
{
    if (self->io == NULL) {
